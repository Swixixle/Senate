...
describe("schema enum lock", () => {
    test("event/receipt schemas contain required enums (drift guard)", () => {
        const eventSchemaPath = node_path_1.default.join(process.cwd(), "src", "schema", "event.schema.json");
        const receiptSchemaPath = node_path_1.default.join(process.cwd(), "src", "schema", "receipt.schema.json");
        const eventSchema = JSON.parse(node_fs_1.default.readFileSync(eventSchemaPath, "utf8"));
        const receiptSchema = JSON.parse(node_fs_1.default.readFileSync(receiptSchemaPath, "utf8"));
